...
class RasterData:
    ...
    def update_geotransform(self):
        ...
        if info.get("transform_coeffs"):
            [xscale, xskew, xoffset,yskew, yscale, yoffset] = info["transform_coeffs"]
        else:
    ...
    # taken from Sean Gillies' "affine.py"
        ...
        if det != 0:
            ...
            raise Exception("Error with the transform matrix, \ a raster should not collapse upon itself")
    ...
    # GET COORDS OF ALL 4 VIEW SCREEN CORNERS
        ...
        mask = self.mask
        # make mask over
        masktrans = mask.transform((width,height), PIL.Image.QUAD,flattened, resample=PIL.Image.NEAREST)
        for band in newraster.bands:
            datatrans = band.img.transform((width,height), PIL.Image.QUAD,flattened, resample=PIL.Image.NEAREST)
